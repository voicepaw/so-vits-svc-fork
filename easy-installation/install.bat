...
if %errorlevel%==0 (
    ...
    python-3.10.10-amd64.exe /quiet InstallAllUsers=0 PrependPath=1
...
if %errorlevel%==0 (
    ...
    if %errorlevel%==0 (
        ...
        echo Please install CUDA 11.7 from https://developer.nvidia.com/cuda-11-7-0-download-archive?target_os=Windows
        exit
    )
    ...
    if exist "C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v11.7\bin\cudnn64_8.dll" (
        ...
        echo Please install cuDNN for CUDA 11.x from https://developer.nvidia.com/cudnn (https://developer.nvidia.com/downloads/compute/cudnn/secure/8.8.1/local_installers/11.8/cudnn-windows-x86_64-8.8.1.3_cuda11-archive.zip/)
        exit
    )
) else (
    set GPU=0
...
if %GPU%==1 (
    ...
    venv\Scripts\pip.exe install torch torchaudio --index-url https://download.pytorch.org/whl/cu116
) else (
